...
DECLARE
    r RECORD;
BEGIN
    FOR r IN SELECT oid::regprocedure FROM pg_proc WHERE proname = 'track_link_click' LOOP
        EXECUTE 'DROP FUNCTION ' || r.oid::regprocedure || ' CASCADE';
    END LOOP;
    FOR r IN SELECT oid::regprocedure FROM pg_proc WHERE proname = 'track_bio_click' LOOP
        EXECUTE 'DROP FUNCTION ' || r.oid::regprocedure || ' CASCADE';
    END LOOP;
    FOR r IN SELECT oid::regprocedure FROM pg_proc WHERE proname = 'is_bot' LOOP
        EXECUTE 'DROP FUNCTION ' || r.oid::regprocedure || ' CASCADE';
    END LOOP;
END $$;

CREATE FUNCTION is_bot(ua TEXT) RETURNS BOOLEAN AS $$
BEGIN
  RETURN (ua ILIKE '%bot%' OR ua ILIKE '%crawl%' OR ua ILIKE '%spider%' OR ua ILIKE '%headless%' OR ua IS NULL OR length(ua) < 10);
END;
$$ LANGUAGE plpgsql;

CREATE FUNCTION track_link_click(
  p_link_id UUID,
  p_ip_address TEXT,
  p_user_agent TEXT,
  p_country TEXT
...
DECLARE
  ...
  v_earnings_amount FLOAT := 0.0015;
BEGIN
  ...
  SET 
    ...
    earnings = CASE WHEN v_is_paid THEN COALESCE(earnings, 0) + v_earnings_amount ELSE earnings END
  WHERE id = p_link_id;
...
CREATE FUNCTION track_bio_click(
  p_profile_id UUID,
  p_ip_address TEXT,
  p_user_agent TEXT,
  p_country TEXT
...
DECLARE
  ...
  v_earned FLOAT := 0;
  v_device TEXT := 'Desktop';
  v_earnings_amount FLOAT := 0.0010;
BEGIN
